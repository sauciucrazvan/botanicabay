const ro = {
  "dashboard_title": "Prezentare Generală a Plantelor",
  "settings": "Setări",
  "settings_choose_theme": "Alege-ți tema",
  "settings_choose_language": "Alege-ți limba",
  "settings_current_version":
      "Folosești în prezent versiunea %version%\n(rulând pe %platform%)",
  "journal": "Jurnal",
  "journal_title": "Jurnalul tău personal",
  "journal_take_notes": "Ia notițe...",
  "journal_disclaimer":
      "Asigură-te că salvezi orice modificare. Revenirea la tabloul de bord nu va salva modificările curente.",
  "journal_update_successful": "Jurnalul a fost actualizat cu succes.",
  "add_plant": "Adaugă o nouă plantă",
  "add_plant_choose_picture": "Să începem - alege o imagine!",
  "add_plant_picture_description":
      "Alege dacă ai deja o imagine a plantei sau vrei să faci una chiar acum.",
  "add_plant_gallery": "Galerie",
  "add_plant_camera": "Cameră",
  "add_plant_invalid_image": "Imaginea pe care ai furnizat-o nu este validă.",
  "add_plant_choose_name": "Dă-i plantei tale un nume...",
  "add_plant_choose_name_description":
      "Acest lucru te va ajuta să o identifici mai târziu. Poți folosi orice dorești - limita este imaginația ta!",
  "add_plant_display_name": "Nume de afișare",
  "add_plant_choose_name_warning": "Trebuie să îi dai plantei tale un nume!",
  "add_plant_custom_variable": "Adaugă o variabilă personalizată",
  "add_plant_custom_variable_description":
      "Poți adăuga variabile personalizate pentru a lua notițe despre fiecare plantă în parte.",
  "variable": "Variabilă",
  "variables": "Variabile",
  "value": "Valoare",
  "finish": "Finalizare",
  "alert": "Alertă",
  "add": "Adaugă",
  "close": "Închide",
  "or": "sau",
  "next_step": "Pasul următor",
  "view_plants_delete_confirmation":
      "Ești sigur că vrei să ștergi '%plant_name%'?",
  "view_plants_delete_notification":
      "'%plant_name%' a fost eliminată cu succes.",
  "view_plants_update_notification":
      "'%plant_name%' a fost actualizată cu succes.",
  "add_variable_invalid_input":
      "Intrarea pe care ai furnizat-o pentru cele 2 casete de text nu este validă.",
  "add_variable_success": "Variabila a fost adăugată cu succes."
};
